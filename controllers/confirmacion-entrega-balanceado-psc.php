...
for ($i = 0; $i < count($fecha_entrega); ++$i) {
    ...
    $select = "SELECT id_piscina, id_corrida, cantidad_balanceado, sobrante, tipo_balanceado FROM solicitud_balanceados WHERE fecha_entrega = '$fecha_entrega' AND camaronera = '$camaronera' AND id = 'Piscina' ORDER BY id_piscina";
        ...
        if ($ban == 0) {
            ...
            for ($i = 0; $i < count($piscina); ++$i) {
                ...
                if ($cantidad > 0) {
                    ...
                    VALUES((NOW()),'$piscina', '$corrida', '$cantidad', '$tipo_alimento', '$camaronera', '$encargado', 'Consumo piscina', 'Piscina', '0')";
