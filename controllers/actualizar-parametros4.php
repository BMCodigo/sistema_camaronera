...
$conexion = $conectar->conectar();
$sqlis ="UPDATE datos_conversion SET estado = '0' WHERE id_tipo= '4' AND id_camaronera='$camaronera';";
$query = mysqli_query($conexion, $sqlis);
 ...
 $sqli ="INSERT INTO datos_conversion (id_biotipo, fecha_registro, factor,bodyweight,rendimiento,estado, id_camaronera) VALUES(4,(NOW()), '$peso', '$factor', '$rendimiento','1','$camaronera');";
$query = mysqli_query($conexion, $sqli);
        }
    }
    $currentname = $_POST['registro'];
   $sqlis ="UPDATE tipos_conversion SET tipo = '$currentname' WHERE id_biotipo = 4 AND id_camaronera='$camaronera';";
$query = mysqli_query($conexion, $sqlis);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['excels']) && !empty($_FILES['excels']['name'][0])) {
        ...
        try {
            ...
            $archivoXLS = $uploadDir . 'file4-'. $diff . '.xlsx'; 
