...
$sql = mysqli_query($conexion, "SELECT * FROM registro_pesca_precria WHERE id_camaronera = '$camaronera' AND estado = 'Sobrante'");
$query = mysqli_num_rows($sql);

$sobrante_mostrar = -100;
$id_sobrante_mostrar = -100;

#mostramos el sobrabte de la precria

if ($query > 0) { ?>
    <div class="card col-9 container" style="display: none;">
        <div class="card-header" style="background: #404e67;">
            <h6 class="text-white text-uppercase" style="margin:auto;"><strong>Precrias con sobrante disponible</strong></h6>
        </div>
        <div class="card-body">
            <div class="dt-responsive">
                <table id="scr-vrt-dt" class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        ...
                        $objeto_tabla_pre = new corrida();
                        $sql_pre = "SELECT * FROM registro_pesca_precria WHERE id_camaronera = '$camaronera' AND estado = 'Sobrante'";
                        $data = $objeto_tabla_pre->mostrar($sql_pre);
                        foreach ($data as $key_pre) {
                            $pre = $key_pre['id_precria'];
                            $id_sobrante_mostrar = $pre;
                        ?>

                            <tr class="text-center">
                                <td><?php echo '# ' . $key_pre['id_precria']; ?></td>
                                <td>
                                    <?php

                                    $objeto = new corrida();
                                    $sql = "SELECT cantidad_siembra FROM registro_piscina_precria WHERE id_camaronera = '$camaronera' AND estado = 'En proceso' and id_precria = '$pre'";
                                    $data = $objeto->mostrar($sql);
                                    foreach ($data as $key) {
                                        echo $smb = intval($key['cantidad_siembra']);
                                    }
                                    ?>
                                </td>
                                <td><?php echo $tnsf = $key_pre['cantidad']; ?></td>
                                <td><?php echo $sobrante_mostrar = intval($smb - $tnsf); ?></td>
                                <td class="text-danger"><span class="badge badge-danger"><?php echo 'Tiene un sobrante' ?></span></td>
...
<div class="container col-md-7 mt-3">
    ...
    <div class="card">
        <div class="card-header" style="background: #404e67;">
            <marquee><h6 class="text-white" style="margin:auto;"><strong>TRANSFERENCIA DE PRECRIA A PISCINA</strong></h6></marquee>
        </div>
        <div class="card-body">


            <div class="mb-20 mt-3">
                <form onsubmit="return pescapre()" action="../controllers/insert-transferencia-pre-ps.php" method="post">
                            ...
                            <div class="col-sm-8 col-lg-7">
                                <div class="input-group">
                                    <select class="form-control" name="piscina_destino[]" id="piscina_destino">
                                        ...
                                        $objeto_tabla_piscina = new corrida();
                                        $sql_tabla_piscina = "SELECT DISTINCT piscinas, hectareas, descripcion_piscina FROM piscina WHERE id_camaronera = '$camaronera' AND id_piscina != 27 ORDER BY descripcion_piscina";
                                        $data = $objeto_tabla_piscina->mostrar($sql_tabla_piscina);
                                        ?>
